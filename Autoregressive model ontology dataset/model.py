...
class MultiHeadSelfAttention(nn.Module):
    ...
    def attention(self, query, key, value, mask=None):
        ...
        if mask is not None:
          # print("Mask dtype before masked_fill:", mask.dtype)  # Should output torch.bool
          mask = mask.bool()
          attention_scores = attention_scores.masked_fill(~mask, float('-inf'))
...
class TransformerModel(nn.Module):
    ...
    def forward(self, x, padding_mask=None):
        ...
        if padding_mask is not None:
          ...
          causal_mask = torch.tril(torch.ones(seq_length, seq_length, device=x.device)).bool().unsqueeze(0).unsqueeze(0)
          # print(f"Casual mask: {causal_mask}")
          mask = padding_mask & causal_mask  # Combine padding mask with causal mask
          # print(f"Mask after combining: {mask}")
          mask = mask.bool()
          # print(f"Mask after bool: {mask}")
        else:
